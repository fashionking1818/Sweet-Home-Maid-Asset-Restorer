...
它能够从服务端抓取散碎的 Spine、图片、音频这些美术相关资源，并根据 `config` 和 `import` 数据将其还原为可读的文件名，同时支持自动提取 Spine 骨骼动画数据和合成 ADV 剧情视频。但是仍有不完善的地方，以及不确定是否存在漏掉的资源😃 。

## 主要功能

* **智能还原文件名**：利用 `config.json` 中的 `paths` 映射，将压缩过的的 UUID (如 `076f221e4`) 还原为真实文件名 (如 `102005_6`) 。
